                    ...
                    __global uint *pHistOut)
{
    ...
    __local ushort hist[THREAD_NUM][BINS+1];
    ...
    #pragma unroll
    for (int i = 0; i < BINS; ++i) {
        int gray_y = groupy * THREAD_NUM + localy;
        int gray_x = groupx * BINS + localx;
    ...
    pHistOut += (groupw * groupy + groupx) * BINS;
    
    /* accumulate histogram for the whole patch */
    ushortN acc = 0;
    #pragma unroll
    for (int i = 0; i < THREAD_NUM; ++i) {
        __local ushort *plocal = &hist[i][localy * N];
        ushortN v = vloadN(0, plocal);
        acc += v;
    }
    vstoreN(acc, localy * N, pHistOut);
}